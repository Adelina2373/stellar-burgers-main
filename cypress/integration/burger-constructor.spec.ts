...
describe('service is available', function () {
	...
	// cypress/integration/burger-constructor.spec.ts
	it('should open ingredient details', function () {
		// Убираем мешающий overlay
		cy.get('iframe#webpack-dev-server-client-overlay').then($iframe => {
			if ($iframe.length) {
				$iframe.remove()
			}
		})
	...
	it('should authorization', function () {
		const email = 'adelina-chekster@mail.ru'
